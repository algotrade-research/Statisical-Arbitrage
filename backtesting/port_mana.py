...
class PortfolioManager:
    ...
    def reset(self):
        ...
        self.positions = {
            "VN30F1M": 0
        }  # Contracts for VN30F1M (positive = long, negative = short)
    ...
    def get_portfolio_value(
        self, vn30f1m_price: float, stock_prices: Dict[str, float]
    ) -> float:
        ...
        """
        stocks_value = sum(
            self.stock_positions.get(stock, 0) * stock_prices.get(stock, 0)
            for stock in self.stock_positions
            if not np.isnan(stock_prices.get(stock, 0))
        )
        futures_value = (
            (vn30f1m_price - self.entry_prices["VN30F1M"])
            * self.positions["VN30F1M"]
            * self.contract_size
    ...
    def _adjust_position(
        self, asset: str, price: float, target_units: int, is_futures: bool
        ...
        """
        current_units = (
            self.positions[asset] if is_futures else self.stock_positions.get(asset, 0)
        )
        ...
        if delta_units > 0:  # Buy
            ...
            if self.cash - total_cost >= min_cash:
                ...
                if asset not in entry_dict:
                    ...
                    avg_entry = (
                        entry_dict[asset] * current_units + price * delta_units
                    ) / total_units
            ...
            if units_to_sell > 0:
                ...
                realized_pnl = (
                    (price - entry_dict[asset])
                    * units_to_sell
                    * (self.contract_size if is_futures else 1)
                )
                ...
                if (is_futures and self.positions[asset] == 0) or (
                    not is_futures and self.stock_positions[asset] == 0
                ):
    ...
    def _calculate_daily_profits(
        self,
        prev_vn30_price: float,
        vn30_price: float,
        prev_stock_prices: Dict[str, float],
        ...
        """
        profit_futures = (
            (vn30_price - prev_vn30_price)
            * self.positions["VN30F1M"]
            * self.contract_size
            if prev_vn30_price is not None and not np.isnan(vn30_price)
            else 0
        )
        profit_stocks = sum(
            (stock_prices[stock] - prev_stock_prices[stock])
            * self.stock_positions[stock]
            for stock in self.stock_positions
            if prev_stock_prices.get(stock) is not None
            and not np.isnan(stock_prices.get(stock, 0))
        )
    ...
    def _record_state(
        ...
        stock_prices: Dict[str, float],
        daily_fee: float,
        profit_stocks_today: float,
        profit_futures_today: float,
    ):
        ...
        vn30_notional = (
            abs(self.positions["VN30F1M"]) * vn30_price * self.contract_size
            if not np.isnan(vn30_price)
            else 0
        )
        abs_stocks = sum(
            self.stock_positions.get(stock, 0) * stock_prices.get(stock, 0)
        ...
        self.asset_df.append(
            {
                ...
                "vn30f1m_notional": vn30_notional,
        ...
        vn30_pct_change = (
            (vn30_price - self.prev_vn30_price) / self.prev_vn30_price
            if self.prev_vn30_price is not None
            and self.prev_vn30_price != 0
            and not np.isnan(vn30_price)
            else 0
        )
        stock_pct_change = (
            sum(
                (
                    (stock_prices[stock] - self.prev_stock_prices[stock])
                    / self.prev_stock_prices[stock]
                )
                * (
                    self.stock_positions.get(stock, 0)
                    * self.prev_stock_prices[stock]
                    / abs_stocks
                )
                for stock in self.stock_positions
                if self.prev_stock_prices.get(stock) is not None
                and self.prev_stock_prices[stock] != 0
                and not np.isnan(stock_prices.get(stock, 0))
                and abs_stocks > 0
            )
            if abs_stocks > 0
        ...
        weights = {
            "vn30f1m_weight": (
                ...
                if portfolio_value > 0 and not np.isnan(vn30_price)
                else 0
            ),
            **{
                f"{stock}_weight": (
                    (
                        self.stock_positions.get(stock, 0)
                        * stock_prices.get(stock, 0)
                        / portfolio_value
                    )
                    if portfolio_value > 0 and not np.isnan(stock_prices.get(stock, 0))
                    else 0
                )
                for stock in stock_prices
            },
        ...
        self.detail_df.append(
            {
                ...
                "cash": self.cash,
                "fee": daily_fee,
                "vn30f1m_pct_change": vn30_pct_change,
                "stock_pct_change": stock_pct_change,
                "profit_stocks_today": profit_stocks_today,
                "profit_futures_today": profit_futures_today,
    ...
    def run_backtest(
        self, position_df: pd.DataFrame, prices: pd.DataFrame
        ...
        stocks = [
            ...
            if col.endswith("_Position") and col != "VN30F1M_Position"
        ]

        for day, date in enumerate(common_dates):
            vn30_price = (
                prices.loc[date, "VN30F1M"] if "VN30F1M" in prices.columns else np.nan
            )
            stock_prices = {
                stock: prices.loc[date, stock] if stock in prices.columns else np.nan
                for stock in stocks
            }
            ...
            if not np.isnan(vn30_price) and vn30_price > 0:
                ...
                target_contracts = (
                    int(
                        (portfolio_value * target_proportion / vn30_price)
                        / self.contract_size
                    )
                    * self.contract_size
                )
                fee_futures, _, contracts_traded_vn30 = self._adjust_position(
                    "VN30F1M", vn30_price, target_contracts, True
                )
            ...
            for stock in stocks:
                ...
                if price is not None and not np.isnan(price) and price > 0:
                    ...
                    target_shares = (
                        int(
                            (portfolio_value * target_proportion / price)
                            / self.contract_size
                        )
                        * self.contract_size
                    )
                    fee_stock, _, traded = self._adjust_position(
                        stock, price, target_shares, False
                    )
                    daily_fee += fee_stock
        ...
        total_fee = sum(entry["fee"] for entry in self.detail_df)
        total_fee_ratio = total_fee / self.initial_balance
        return (
            pd.DataFrame(self.asset_df).set_index("Date"),
